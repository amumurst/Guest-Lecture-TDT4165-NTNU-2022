...
object _Intro {
  ...
  object Definitions {
    ...
    /** * Loops
      *
      * In pure functional programing we do not have constructs such as a for-loop or a while-loop since they are
      * inherently mutating state
      *
    ...
    for (s <- list1) stringListMutable.appended(s.toString)
  ...
  object Objects {
    ...
    val foo = FooClass(1, 2)

    /** * Case class gives us:
      *   1. A default constructor 2. All parameters are public 3. A correct hash function 4. An equals method that
      *      checks equality correctly 5. Copy functionality
      */
    case class BarClass(x: Int, y: Int)
    val anInstance      = BarClass(2, 1)
  ...
  object Domains {
    ...
    //val myThing: Nothing = ??? //Impossible!
    //Unit - A type with a single member, itself, Unit.
    val x: Unit = ()
    //Any - A supertype of all other types
    //Very few functions take Any, and signs of Any in code is often a code smell
    def foo(f: Any): Any = ???

    //So for a given class Foo we have Nothing <: Foo <: Any (<: means is a subtype of)
    ...
    val listContains = pythonesqueList.map {
      case _: Int     => "int"
