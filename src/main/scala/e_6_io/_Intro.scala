...
object _Intro {
  ...
  /** * IO is a datatype that describes a program eg. something that can be executed Nothing is executed at creating. We
    * then collect and combined descriptions of programs and execute them only in the main function of our app
    */
  ...
  val forComprehension: IO[Int] =
    power12IO.flatMap(p1 => power12IO.map(p2 => p1 + p2))
